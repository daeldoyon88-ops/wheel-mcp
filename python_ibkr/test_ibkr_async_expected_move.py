"""
Smoke test read-only: expected move type Barchart (60% ATM, 30% strangle±1, 10% strangle±2) via IBKR.
Aucun ordre; connect uniquement avec StartupFetchNONE.
"""
...
def _conservative_premium(bid, ask, last, close) -> tuple[float | None, str | None]:
    """mid si bid+ask valides, sinon prudence: last, bid, close."""
...
def _ticker_quotes(ticker) -> dict:
...
def main() -> int:
    ...
    client_id = _int_env("IBKR_CLIENT_ID", 210)
    ...
    try:
        ...
        def _try_series(
            ...
            """
            Tente d'ouvrir chaque strike dans l'ordre, max MAX_STRIKE_ATTEMPTS.
            Retourne (contract, strike_ok, attemptedStrikes, selectedAfterAttempts) ou (None, None, attempted, 0).
            """
        ...
        # Strangle1 call : calls au-dessus de l'ATM (strikes plus élevés)
        s1c_series = [strikes[i] for i in range(i_atm + 1, min(len(strikes), i_atm + 1 + MAX_STRIKE_ATTEMPTS))]
        c_s1c, s_call_p1, a_s1c, n_s1c = _try_series("strangle1_call", "C", s1c_series)

        # Strangle1 put : puts en-dessous (strikes plus bas)
        ...
        # Strangle2 : seulement après s1, strikes différents
        if s_call_p1 is not None:
            i_s1c = _strike_index(s_call_p1)
            if i_s1c >= 0 and i_s1c + 1 < len(strikes):
                s2c_series = [strikes[i] for i in range(i_s1c + 1, min(len(strikes), i_s1c + 1 + MAX_STRIKE_ATTEMPTS))]
                c_s2c, s_call_p2, a_s2c, n_s2c = _try_series("strangle2_call", "C", s2c_series)
        if s_put_m1 is not None:
            i_s1p = _strike_index(s_put_m1)
            if i_s1p > 0:
                s2p_series = [strikes[i_s1p - k] for k in range(1, min(MAX_STRIKE_ATTEMPTS + 1, i_s1p + 1))]
                c_s2p, s_put_m2, a_s2p, n_s2p = _try_series("strangle2_put", "P", s2p_series)

        tickers_map: dict[tuple[str, str, float], object] = {}
        meta: dict[tuple[str, str, float], dict] = {}

        def _register(role: str, rght: str, k: float | None, c, att: list, n: int) -> None:
        ...
        if c_atm_c is not None and s_atm_c is not None and n1:
            _register("atm_call", "C", s_atm_c, c_atm_c, _a1, n1)
        if c_atm_p is not None and s_atm_p is not None and n2:
            _register("atm_put", "P", s_atm_p, c_atm_p, _a2, n2)
        if c_s1c is not None and s_call_p1 is not None and n_s1c:
            _register("strangle1_call", "C", s_call_p1, c_s1c, a_s1c, n_s1c)
        if c_s1p is not None and s_put_m1 is not None and n_s1p:
            _register("strangle1_put", "P", s_put_m1, c_s1p, a_s1p, n_s1p)
        if c_s2c is not None and s_call_p2 is not None and n_s2c:
            _register("strangle2_call", "C", s_call_p2, c_s2c, a_s2c, n_s2c)
        if c_s2p is not None and s_put_m2 is not None and n_s2p:
            _register("strangle2_put", "P", s_put_m2, c_s2p, a_s2p, n_s2p)

        ib.sleep(5.0)

        def _row(role: str, rght: str, k: float):
            t = tickers_map.get((role, rght, float(k)))
            if t is None:
                return None
            q = _ticker_quotes(t)
            m = meta.get((role, rght, float(k)), {})
            options_out.append(
                {
                    "role": role,
                    "right": rght,
        ...
        def _p(role: str, rght: str, k: float | None) -> float | None:
            if k is None:
                return None
            return _row(role, rght, k)

        p_atm_c = _p("atm_call", "C", s_atm_c) if c_atm_c else None
        p_atm_pu = _p("atm_put", "P", s_atm_p) if c_atm_p else None
        p_s1_c = _p("strangle1_call", "C", s_call_p1)
        p_s1_p = _p("strangle1_put", "P", s_put_m1)
        p_s2_c = _p("strangle2_call", "C", s_call_p2)
        p_s2_p = _p("strangle2_put", "P", s_put_m2)
        ...
        if True:
            parts.append((W_ATM, atm_straddle_val))

"""
Smoke test: lecture d'une quote action via ib_async (lecture seule, aucun ordre).
Ne demande ni compte/positions/ordres/exécutions.
"""
...
def main() -> int:
    ...
    client_id = _int_env("IBKR_CLIENT_ID", 202)
    read_only = _parse_bool(os.environ.get("IBKR_READ_ONLY"), True)
    symbol = _str_env("IBKR_SYMBOL", "SOFI").upper()
    ...
    try:
        ...
        # readonly=True si supporté; fallback propre si version sans ce paramètre.
        try:
        ...
        out = {
            ...
            "marketDataType": market_data_type,
            "contract": {
                "secType": _safe_attr(contract, "secType") or "STK",

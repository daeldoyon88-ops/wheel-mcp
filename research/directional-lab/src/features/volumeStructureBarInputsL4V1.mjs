 ...
 */
export function toInternalVolumeStructureBars(rows) {
  if (!Array.isArray(rows)) throw new TypeError('rows must be an array');
  return rows.map((row) => ({
    source: row,
    open: fixedFromCanonical({ atoms: row.openAtoms, scale: row.priceScale }),
    high: fixedFromCanonical({ atoms: row.highAtoms, scale: row.priceScale }),
    low: fixedFromCanonical({ atoms: row.lowAtoms, scale: row.priceScale }),
    close: fixedFromCanonical({ atoms: row.closeAtoms, scale: row.priceScale }),
    volume: row.volumeAtoms === null
      ? null
      : fixedFromCanonical({ atoms: row.volumeAtoms, scale: row.volumeScale }),
  }));
}

/** @param {{value: unknown, availability: string}} cell */
function toInternalCell(value, availability) {
  if (availability === 'AVAILABLE') {
    return { value: fixedFromCanonical(value, FEATURE_CALCULATION_SCALE), availability };
  }
 ...
 */
export function extractTechnicalCells(technicalRows) {
  if (!Array.isArray(technicalRows)) throw new TypeError('technicalRows must be an array');
  return technicalRows.map((row) => ({
    atr14: toInternalCell(row.features.volatility.atr14, row.availability.volatility.atr14),
    atr14Pct: toInternalCell(row.features.volatility.atr14Pct, row.availability.volatility.atr14Pct),
    return20: toInternalCell(
      row.features.returnsDrawdowns.return20, row.availability.returnsDrawdowns.return20,
    ),
  }));
}

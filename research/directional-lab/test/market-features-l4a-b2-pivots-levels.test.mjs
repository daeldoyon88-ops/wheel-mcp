...
function pivotsFor(bars) {
  return detectConfirmedPivots(bars, MARKET_VOLUME_STRUCTURE_RUNTIME_POLICY_V1.pivots);
}

function statesFor(bars) {
  return computeAlternatedStreamStates(
    bars, pivotsFor(bars), MARKET_VOLUME_STRUCTURE_RUNTIME_POLICY_V1.pivots,
  );
}

function supportResistanceRows(bars) {
  return computeSupportResistanceFeatures(
    bars, pivotsFor(bars), makeTechnicalCellsFromBars(bars),
    MARKET_VOLUME_STRUCTURE_RUNTIME_POLICY_V1.supportResistance,
  );
}
...
test('L4A-B2 the pivot family stays NO_CONFIRMED_PIVOT until the confirmation row', () => {
  ...
  const bars = makeVolumeBars(closes, { spread: 1n });
  const rows = computePivotFamilyRows(statesFor(bars), MARKET_VOLUME_STRUCTURE_RUNTIME_POLICY_V1.pivots);

...
test("BALANCED — période 0,69 % → fallback admis avec statut BELOW", () => {
  const pool = [makeCandidate({ ticker: "CRM", safe: makeLeg({ strike: 100, periodYieldPct: 0.69, dteDays: 7 }) })];
  const { combos, holder } = runBuckets(pool);
  const pick = pickOf(combos, "BALANCED", "CRM");
  assert.ok(pick);
  assert.equal(pick.balancedLegDiagnostics.selectedYieldBandStatus, "BELOW");
  assert.equal(rejectsOf(holder, "BALANCED").length, 0);
  approx(pick.balancedLegDiagnostics.effectivePeriodMinPct, 0.7, 0.0001);
});
...
test("BALANCED — période 1,05 % → fallback admis avec statut ABOVE", () => {
  ...
  assert.equal(rejectsOf(holder, "BALANCED").length, 0);
  approx(pick.balancedLegDiagnostics.effectivePeriodMaxPct, 1.05, 0.0001);
});
